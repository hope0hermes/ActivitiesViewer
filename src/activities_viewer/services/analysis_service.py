...
class AnalysisService:
    ...
    def get_pmc_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Performance Management Chart (PMC) data: CTL, ATL, TSB.
        ...
        Returns:
            DataFrame with columns: date, ctl, atl, tsb
        """
        if df.empty:
            return pd.DataFrame(columns=["date", "ctl", "atl", "tsb"])

        # Prepare daily TSS data
        df_copy = df.copy()
        df_copy["start_date_local"] = pd.to_datetime(df_copy["start_date_local"])
        if df_copy["start_date_local"].dt.tz is not None:
            df_copy["start_date_local"] = df_copy["start_date_local"].dt.tz_localize(
                None
            )
        ...
        # Create continuous date range (fill missing days with 0 TSS)
        if len(daily_tss) < 2:
            return pd.DataFrame(columns=["date", "ctl", "atl", "tsb"])

        date_range = pd.date_range(
            start=daily_tss["date"].min(), end=daily_tss["date"].max(), freq="D"
        )

        full_dates = pd.DataFrame({"date": date_range})
        daily_tss = full_dates.merge(daily_tss, on="date", how="left")
        daily_tss["tss"] = daily_tss["tss"].fillna(0)

        # Calculate exponentially weighted moving averages
        # CTL: 42-day EWM (alpha = 2/(42+1) ≈ 0.0465)
        # ATL: 7-day EWM (alpha = 2/(7+1) = 0.25)
        ...
        return daily_tss[["date", "ctl", "atl", "tsb"]]
